...
const serializeObj = async (splats: Splat[], options: SerializeSettings, writer: Writer) => {
    const filter = new GaussianFilter(options);
    const totalGaussians = countGaussians(splats, filter);
    ...
    const header = `# Generated by SuperSplat\n`;
    ...
    for (let e = 0; e < splats.length; ++e) {
        ...
        for (let i = 0; i < splatData.numSplats; ++i) {
            ...
            const x = data.x.toFixed(6);
            const y = data.y.toFixed(6);
            const z = data.z.toFixed(6);
            const r = clamp(0.5 + SH_C0 * data.f_dc_0).toFixed(6);
            const g = clamp(0.5 + SH_C0 * data.f_dc_1).toFixed(6);
            const b = clamp(0.5 + SH_C0 * data.f_dc_2).toFixed(6);
...
const serializeFbx = async (splats: Splat[], options: SerializeSettings, writer: Writer) => {
    const filter = new GaussianFilter(options);
    const totalGaussians = countGaussians(splats, filter);
    if (totalGaussians === 0) return;
    ...
    const header = `; FBX 7.5.0 project file
GlobalSettings:  {
	Version: 1000
	Properties70:  {
...
Objects:  {
    ...
    for (let e = 0; e < splats.length; ++e) {
        ...
        for (let i = 0; i < splatData.numSplats; ++i) {
            ...
            singleSplat.read(splat, i);
            if (!first) chunk += ',';
            chunk += `${data.x.toFixed(6)},${data.y.toFixed(6)},${data.z.toFixed(6)}`;
            first = false;
            if (chunk.length > 100000) {
                await writer.write(new TextEncoder().encode(chunk));
                chunk = '';
    ...
    if (chunk.length > 0) await writer.write(new TextEncoder().encode(chunk));
    ...
    const midSection = `
		...
		LayerElementNormal: 0 {
			...
			Normals: *${totalGaussians * 3} {
				a: `;
    await writer.write(new TextEncoder().encode(midSection));
    ...
    const colorSection = `
		...
		LayerElementColor: 0 {
			...
			MappingInformationType: "ByControlPoint"
			ReferenceInformationType: "Direct"
			Colors: *${totalGaussians * 4} {
				a: `;
    await writer.write(new TextEncoder().encode(colorSection));
